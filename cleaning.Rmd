...
gss_cross <- gss_cross %>% 
  mutate(across(all_of(knowledge_variables), ~ ifelse(. == 1, TRUE, FALSE)))
```
...
(`earthsun` and `solarrev` are not TRUE/FALSE question, we did not take them in account.)
...
reshape_data <- function(df, panel_name) {
  ...
  reduced <- df %>%
    select(starts_with("id_1"), starts_with("year_"), starts_with("consci"), starts_with(c("hotcore", "radioact", "boyorgrl", "lasers", "electron", "viruses", "bigbang", "condrift","evolved"))) %>%
...
gss_panel_cleaned <- gss_panel  %>% 
  ...
  select(names(gss_panel)) %>% 

  #in panel_08, evolved1 variable has a different name
  rename(evolved1 = EVOLVED1)  %>% 
  
  # there are 2 variable names for the same questions, bigbang1 and BIGBANG1 (panel_08).
  mutate(bigbang1 = coalesce(bigbang1, BIGBANG1)) %>% 
  select(-BIGBANG1) %>% 
  
  # TRUE,FALSE instead of 1,2
  mutate_at(vars(c(starts_with(c("hotcore", "radioact", "boyorgrl", "lasers", "electron", "viruses", "bigbang", "condrift","evolved")))), ~if_else(. == 1, TRUE, FALSE))
